  ...
  try {
    ...
    if (res.no_chrome) {
      ...
      availabilityCache = {
        ...
        ipv4Total: 0, ipv4Timeout: 0, ipv6Total: 0, ipv6Timeout: 0,
        preferred: null,
...
<template>
  <Teleport to="body">
    <div v-if="open" class="fixed inset-0 z-[990] flex items-center justify-center p-4">
      ...
      <!-- Dialog -->
      <div class="relative bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-lg border border-slate-200 dark:border-slate-700 animate-in zoom-in-95 duration-200 overflow-hidden">
        ...
        <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-slate-700">
          <div class="flex items-center gap-3">
            <div :class="[
              ...
              mode === 'lan' ? 'bg-emerald-100 dark:bg-emerald-900/40 text-emerald-600 dark:text-emerald-400'
          ...
          <div class="flex items-center gap-2">
            <button @click="handleRefresh" :disabled="loading || checkingAvailability"
        ...
        <div class="p-6">
          ...
          <div v-else class="grid grid-cols-2 gap-6 items-stretch">
            ...
            <div :class="[
              ...
              mode === 'public' && preferred === 'ipv4' ? 'ring-2 ring-blue-400/60 bg-blue-50/30 dark:bg-blue-900/10' : ''
            ]">
              <div class="flex items-center gap-1.5">
                ...
                <!-- 推荐徽章 -->
                <span v-if="mode === 'public' && preferred === 'ipv4' && availabilityChecked"
              ...
              <div v-else class="w-[160px] h-[160px] bg-slate-50 dark:bg-slate-900 rounded-xl border border-dashed border-slate-300 dark:border-slate-600 flex items-center justify-center">
              ...
              <div v-if="ipv4" class="w-full space-y-1.5 flex flex-col flex-1 justify-end">
                <div class="px-3 py-2 bg-slate-50 dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-700 text-xs text-slate-600 dark:text-slate-300 font-mono text-center break-all select-all min-h-[2.75rem] flex items-center justify-center">
                  {{ buildUrl(ipv4, port) }}
                </div>
                <!-- 可用性检测结果 (仅公网模式) -->
            ...
            <div :class="[
              'flex flex-col items-center gap-3 rounded-xl p-2 -m-2 transition-all',
              mode === 'public' && preferred === 'ipv6' ? 'ring-2 ring-blue-400/60 bg-blue-50/30 dark:bg-blue-900/10' : ''
            ]">
              <div class="flex items-center gap-1.5">
                <p class="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">
                  {{ t('networkLink.ipv6Label') }}
                </p>
                <span v-if="mode === 'public' && preferred === 'ipv6' && availabilityChecked"
                  class="px-1.5 py-0.5 rounded-full bg-blue-500 text-white text-[9px] font-bold leading-none">
                  {{ t('networkLink.recommended') }}
                </span>
              </div>
              ...
              <div v-if="ipv6" class="w-full space-y-1.5 flex flex-col flex-1 justify-end">
                ...
                <div class="flex flex-col gap-1.5">
                  <button @click="copyUrl(ipv6)"
                    class="w-full inline-flex items-center justify-center gap-1.5 py-1.5 rounded-lg bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 text-xs font-medium hover:bg-slate-200 dark:hover:bg-slate-600 transition-all active:scale-95 leading-none">
                    <PhCopy :size="13" class="shrink-0" />
                    {{ t('common.copy') }}
                  </button>
                  <button @click="openBrowser(ipv6)"
                    class="w-full inline-flex items-center justify-center gap-1.5 py-1.5 rounded-lg bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 text-xs font-medium hover:bg-slate-200 dark:hover:bg-slate-600 transition-all active:scale-95 leading-none">
                    <PhGlobe :size="13" class="shrink-0" />
                    {{ t('networkLink.openInBrowser') }}
                  </button>
          ...
          <div class="mt-4 text-center">
            ...
            <div v-if="mode === 'public' && noChrome" class="space-y-2">
              <p class="text-xs text-amber-500 dark:text-amber-400 font-medium flex items-center justify-center gap-1.5">
                <PhWarning :size="13" class="shrink-0" />
                未检测到 Chrome，无法自动检测可用性
              </p>
              <div class="flex items-center justify-center gap-2 flex-wrap">
                <button v-if="itdogUrlV4"
            ...
            <!-- 检测进行中 -->
            <p v-else-if="mode === 'public' && checkingAvailability" class="text-xs text-slate-400 dark:text-slate-500 flex items-center justify-center gap-1.5">
