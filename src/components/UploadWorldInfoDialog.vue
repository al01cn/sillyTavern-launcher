...
watch(() => uploadWorldInfoState.show, (newVal) => {
    if (newVal) {
        resetState();
    }
});
...
<template>
    <div :class="[
        ...
        <div class="absolute inset-0 bg-slate-900/40 backdrop-blur-md"
            @click="!isImporting && closeUploadWorldInfoDialog()"></div>
        ...
        <div :class="[
            ...
            <div class="px-6 py-5 border-b border-slate-100 flex items-center justify-between">
                ...
                <button @click="!isImporting && closeUploadWorldInfoDialog()" :disabled="isImporting"
            ...
            <div class="p-6 space-y-6">
                ...
                <div v-if="selectedFiles.length === 0" @dragover="onDragOver" @dragleave="onDragLeave" @drop="onDrop">
                    <label class="block text-sm font-bold text-slate-700 mb-2">{{ t('resources.worldJsonFile')
                        }}</label>
                ...
                <div v-else-if="selectedFiles.length === 1">
                    ...
                    <div @click="selectFile" :class="[
                        ...
                        <div class="flex flex-col items-center pointer-events-none">
                            <div
                                class="w-12 h-12 rounded-full bg-blue-100 flex items-center justify-center text-blue-500 mb-3">
                    ...
                    <div class="mt-6 rounded-xl p-4 transition-all" :class="[
                        ...
                        <div v-else-if="selectedFiles[0].status === 'invalid'"
                            class="flex items-start gap-3 text-red-600">
                        ...
                        <div v-else-if="selectedFiles[0].status === 'success'"
                            class="flex items-center gap-3 text-emerald-600">
                        ...
                        <div v-else-if="selectedFiles[0].status === 'valid'"
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <div class="mt-2 space-y-1 text-xs opacity-90">
                                    <p v-if="selectedFiles[0].worldName"><span class="font-semibold">{{
                                            t('resources.worldName') }}：</span>{{ selectedFiles[0].worldName }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div v-else-if="selectedFiles.length > 1" class="space-y-4">
                    <div class="flex items-center justify-between">
                        <label class="block text-sm font-bold text-slate-700">{{ t('resources.selectedFiles', {
                            count: selectedFiles.length }) }}</label>
                        ...
                        <div v-for="file in selectedFiles" :key="file.id"
                            ...
                            <div class="flex items-center gap-3 min-w-0 flex-1">
                                ...
                                <div class="min-w-0 flex-1">
                                    <p class="text-sm font-bold text-slate-700 truncate"
                                        :title="file.worldName || file.name">
                            ...
                            <div class="shrink-0 ml-3 flex items-center justify-end min-w-[70px]">
                                <div v-if="file.status === 'verifying'"
                                    class="flex items-center gap-1.5 text-slate-400 text-xs">
                                ...
                                <div v-else-if="file.status === 'valid'"
                                    class="flex items-center gap-1.5 text-emerald-500 text-xs">
                                    <CheckCircle2 class="w-3.5 h-3.5" />
                                    <span>{{ t('resources.pending') }}</span>
                                </div>
                                <div v-else-if="file.status === 'importing'"
                                    class="flex items-center gap-1.5 text-blue-500 text-xs">
                                    <Loader2 class="w-3.5 h-3.5 animate-spin" />
                                ...
                                <div v-else-if="file.status === 'error'"
                                    class="flex items-center gap-1.5 text-red-500 text-xs" :title="file.errorMsg">
                                    <X class="w-3.5 h-3.5" />
            ...
            <div class="p-5 border-t border-slate-100 bg-slate-50 flex justify-end gap-3">
                <button @click="closeUploadWorldInfoDialog" :disabled="isImporting"
                    class="px-5 py-2.5 rounded-xl font-bold text-slate-500 hover:bg-slate-200 transition-colors disabled:opacity-50 text-sm">
                    {{ t('common.cancel') }}
                </button>
                <button @click="importWorlds" :disabled="!hasValidFiles || isAnyVerifying || isImporting"
