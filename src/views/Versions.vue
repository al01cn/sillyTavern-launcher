<template>
  <div class="space-y-6">
      ...
      >
        <div>
          ...
          <div class="flex items-baseline space-x-2">
            ...
            <span
              v-if="currentVersion"
              class="text-[10px] px-2 py-0.5 rounded-full font-bold uppercase tracking-wide flex-shrink-0"
              :class="
                currentLocalPath
          ...
          <p
            ...
            class="text-[10px] text-slate-400 dark:text-slate-500 mt-1 flex items-center max-w-[200px] truncate"
            :title="currentLocalPath"
          >
    ...
    <div class="flex gap-6 border-b border-slate-200 dark:border-slate-700">
      ...
      >
        ...
        <div
          ...
          class="absolute bottom-[-1px] left-0 w-full h-0.5 bg-blue-600 dark:bg-blue-400 rounded-t-full"
        ></div>
      </button>
      <button
        id="tab-online"
    ...
    >
      <div class="p-5 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between">
        <div class="flex items-center gap-3">
          ...
          <span
            ...
            class="text-[10px] text-blue-500 bg-blue-50 dark:bg-blue-900/30 px-2 py-0.5 rounded-full flex items-center gap-1"
          >
            <Loader2 class="w-3 h-3 animate-spin" />
            {{ t('versions.scanning') }} {{ scanManager.state.scanTimeText }}
          </span>
          <span
            v-else-if="scanManager.state.progress.key"
            class="text-[10px] text-slate-400 dark:text-slate-500 bg-slate-50 dark:bg-slate-700/50 px-2 py-0.5 rounded-full border border-slate-100 dark:border-slate-600"
          >
            {{
              t(scanManager.state.progress.key, {
                count: scanManager.state.progress.count,
                found: scanManager.state.progress.found,
              })
            }}
          </span>
        </div>
      ...
      <div
        v-if="scanManager.state.localList.length === 0 && !scanManager.state.isScanning"
        class="p-10 text-center text-slate-400 dark:text-slate-500 flex flex-col items-center justify-center"
      >
        <FolderSearch class="w-12 h-12 text-slate-300 dark:text-slate-600 mb-3" />
        <p>{{ t('extensions.noExtensionsFound') }}</p>
        <button
          class="mt-4 px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white rounded-lg text-sm font-bold transition-colors shadow-sm"
          @click="startScan(true)"
        >
      ...
      <div v-else class="divide-y divide-slate-50 dark:divide-slate-700">
        <div
          v-for="item in scanManager.state.localList"
          :key="item.path"
          ...
          <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              ...
              <div>
                <div class="flex items-center gap-2">
                  <span class="font-bold text-slate-800 dark:text-slate-200">{{
                    item.version === 'unknown' ? t('versions.unknownVersion') : 'v' + item.version
                  }}</span>
                  <span
            ...
            <div class="flex flex-wrap sm:flex-nowrap items-center gap-2 mt-2 sm:mt-0 shrink-0">
              ...
              <button
                v-else
                :disabled="
                  (currentVersion === item.version && currentLocalPath === item.path) || switchingVersion !== null
                "
              ...
              >
                ...
                {{
                  currentVersion === item.version && currentLocalPath === item.path
                    ? t('versions.currentlyUsed')
                    : switchingVersion === item.path
    ...
    <Teleport to="body">
      <div
        ...
        @click.self="scanLogVisible = false"
      >
        <div
          class="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 w-full max-w-3xl max-h-[80vh] mx-4 flex flex-col animate-in fade-in zoom-in-95 duration-200"
        >
          <div
            class="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-slate-700 flex-shrink-0"
          >
            ...
            <div class="flex items-center gap-3">
              ...
              <button
                class="text-slate-400 hover:text-blue-500 dark:text-slate-500 dark:hover:text-blue-400 transition-colors p-1 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700"
              ...
              <button
                class="text-slate-400 hover:text-slate-600 dark:text-slate-500 dark:hover:text-slate-300 transition-colors p-1 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700"
                @click="scanLogVisible = false"
              >
                <X class="w-4 h-4" />
              </button>
            </div>
          ...
          <div class="flex-1 overflow-auto px-6 py-4 min-h-0">
            <div
              v-if="scanLogRefreshing && scanManager.state.scanLogPaths.length === 0"
              class="flex items-center justify-center py-10 text-slate-400 dark:text-slate-500"
            >
              <Loader2 class="w-5 h-5 animate-spin mr-2" />
              {{ t('versions.scanLogLoading') }}
            </div>
            <div
              v-else-if="scanManager.state.scanLogPaths.length === 0"
              class="flex items-center justify-center py-10 text-slate-400 dark:text-slate-500"
            >
            ...
            <div
              v-else
              ref="scanLogContainer"
              class="text-xs font-mono text-slate-600 dark:text-slate-400 bg-slate-50 dark:bg-slate-900 rounded-xl p-4 overflow-auto max-h-[60vh]"
            >
              <div
                v-for="(p, i) in scanManager.state.scanLogPaths"
                :key="i"
                class="leading-relaxed break-all whitespace-nowrap overflow-hidden text-ellipsis"
                :title="p"
              >
                <span class="text-slate-400 dark:text-slate-600 select-none mr-2">{{
                  String(i + 1).padStart(4, ' ')
                }}</span>
                <span class="text-slate-500 mr-1">{{ t('versions.scanningDir') }}</span
                >{{ p }}
              </div>
            </div>
          </div>
...
// 扫描过程中自动滚动到底部
watch(
  () => scanManager.state.scanLogPaths.length,
  () => {
...
const handleSelectExistingTavern = async () => {
  try {
    ...
    // Parse version
    const dirPath = packageJsonPath.substring(
      0,
      packageJsonPath.lastIndexOf(packageJsonPath.includes('\\') ? '\\' : '/'),
    )
...
const handleSwitchLocal = async (item: LocalTavernItem) => {
  ...
  try {
    ...
    toast.success(
      t('versions.switchedTo', {
        version: item.version === 'unknown' ? t('versions.unknownVersion') : 'Local v' + item.version,
      }),
    )
  } catch (e) {
